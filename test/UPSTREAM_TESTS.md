...
## Utreexod v0.6 transaction proofs

`data/utreexod_tx_vectors.h` is generated by `integration/utreexod_tx_vectors.go`
against utreexod v0.6.0 commit `fe71f3d9282ef0812f7f6087f0c0df9ce0fda508`
and its utreexo v0.18.0 dependency. The 20 cases cover exact transaction inventory,
position requests, and proof-first responses, including witnesses, unconfirmed
inputs, promoted targets in the 63-row position space, all compact script types,
and requested hash order. The C++ tests verify the Go full proofs against Go roots
as well as byte-for-byte wire compatibility and restored transaction identities.

See [the codec documentation](../doc/transaction-proof-codec.md) for regeneration
and the dedicated libFuzzer target. The separate
[`core_utreexod_relay.py`](integration/core_utreexod_relay.py) integration runs
Core 31.1 and the pinned compact v0.6 consumer with the supplied compatibility
patch. It verifies live proof-aware mempool admission, corrupted-proof rejection,
connection/restart recovery, requested hash selection, `getblocktemplate`, standard
`submitblock`, block relay in both directions, and matching accumulator roots.
The [relay guide](../doc/core-backed-transaction-proof-relay.md) gives exact commands.
