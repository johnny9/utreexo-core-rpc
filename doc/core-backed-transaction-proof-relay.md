...
On a `getdata` cache miss, the peer queues a regeneration job and waits for its
result. The sync thread prioritizes up to four requested preparations per poll,
rechecking Core membership and input metadata, then generating and verifying the
proof from the current forest. It answers the original request with `utreexotx`
when possible, including partial and zero-additional-hash requests. Background
Core inventory scans continue recovering other expired or evicted preparations.

Per-peer announcement history retains fixed-size identities, bounded by
`--tx-cache-entries`; it does not pin the expired transaction/proof payloads.
The identity commits to witnesses, targets, all proof hashes, leaf metadata and
the cache epoch. A regenerated preparation must match it exactly. A changed
identity or tip requires reconnecting before reannouncement, so an old partial
request cannot silently select nodes from a different proof.
...
The current consumer changes ship with sidecar v0.6.1. Use its bundled patch or the
...
It uses a one-second preparation lifetime and requires an expired transaction
proof to be regenerated for an existing peer's original request.
